...
router.get('/:id', (req, res, next)=>{
  DualTaskDefinition.findById(req.jwt, req.params.id)
    .then(sd=>{
...
router.delete('/:id', (req, res, next)=>{
  DualTaskDefinition.deleteById(req.jwt, req.params.id)
    .then(()=>{
       res.status(200).send();
    })
    .catch(err=>{
...
router.post('/', (req, res, next)=>{
  DualTaskDefinition.create(req.jwt, req.body)
    .then(sd=>{
      res.status(200).send(sd);
    })
    .catch(err=>{
...
router.patch('/:id', (req, res, next)=>{
  ...
  DualTaskDefinition.updateById(req.jwt, data)
   .then(sd=>{
     res.status(200).send(sd);
   })
   .catch(err=>{
...
router.get('/:id/humantasks', (req, res, next)=>{
  DualTask.findByTaskDefinitionId(req.jwt, req.params.id)
    .then(sd=>{
        res.status(200).send(sd);
    })
    .catch(err=>{
